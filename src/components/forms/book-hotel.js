...
const BookHotel = ({
    ...
    const reserve = async () => {
        ...
        setErrors({})
        setMessage(null)
        await axios.post("/hajj-umrah/hotels/reservations/store", {
            ...
            router.push(`/hajj-and-umrah/hotel/reservations?_r=${reservation}`)
        }).catch(err => {
            setLoading(false)
    ...
    return (
        <Modal size="lg" id="reserveHotelModal" static onClose={() => {
            ...
            <Modal.Body>
                ...
                <div className="mb-3">
                    <Input type="number" name="amount" min={minAmount} max={hotel.packages_left} label="Package Amount" value={data.amount} onChange={(e) => handleChange('amount', e.target.value)} noDecimal />
            ...
            <Modal.Footer className="flex items-center justify-end space-x-3">
                ...
                ) || (Number(data.amount) < minAmount || Number(data.amount) > hotel.packages_left) || minAmount === 0} className="btn-rose" onClick={reserve}>Reserve</button>
            </Modal.Footer>
        </Modal>
    );
};

...
const BookFlight = ({
    ...
    const minSeats = flight.available_seats > 15 ? 15 : flight.available_seats
    ...
    const reserve = async () => {
        ...
        setErrors({})
        setMessage(null)
        await axios.post("/hajj-umrah/flights/reservations/store", {
            ...
            router.push(`/hajj-and-umrah/flight/reservations?_r=${reservation}`)
        }).catch(err => {
            setLoading(false)
    ...
    return (
        <Modal size="lg" id="reserveFlightModal" static onClose={() => {
            setSelectedFlight(null)
        }}>
            <Modal.Header>
                <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Reserve Flight</h4>
            </Modal.Header>
            <Modal.Body>
                {
                    pool && (
            ...
            <Modal.Footer className="flex items-center justify-end space-x-3">
                ...
                ) || (Number(data.seats) < minSeats || Number(data.seats) > flight.available_seats) || minSeats === 0} className="btn-rose" onClick={reserve}>Reserve</button>
            </Modal.Footer>
        </Modal>
    );
};

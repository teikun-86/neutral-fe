...
export const HajjUmrahReservation = ({
    ...
    const reload = async () => {
        setLoading(true)
        await axios.get(`/hajj-umrah/flights/reservations/?id=${res.id}`).then(res => {
            setLoading(false)
    ...
    const canEdit = moment(reservation.flight.depart_at).diff(moment(), 'days') >= 45
                ...
                <div className="w-full flex items-end justify-between">
                    <div className="w-full">
                        ...
                        <h6 className="text-sm font-semibold text-gray-800 dark:text-gray-100">
                            {__('depart_at', {
                                date: moment(reservation.flight.depart_at).format('DD MMMM YYYY'),
                            })} {reservation.flight.departure_airport.iata} - {reservation.flight.arrival_airport.iata}
                        </h6>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{reservation.flight.airline.code}-{reservation.flight.flight_number} ({reservation.seats} {__('seats')})</p>
                        ...
                        <div className="mb-2">
                            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{__('flight.reservation.price_per_seat')}: {formatIDR(reservation.flight.price)}</p>
                            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{__('flight.reservation.seats')}: {reservation.seats} {__('seats')}</p>
                        </div>
                    </div>

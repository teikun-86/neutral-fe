...
export const HotelReservation = ({
    ...
    const reload = async () => {
        setLoading(true)
        await axios.get(`/hajj-umrah/hotels/reservations/?id=${res.id}`).then(res => {
            setLoading(false)
    ...
    const canEdit = moment(reservation.hotel.first_check_in_at).diff(moment(), 'days') >= 45
                ...
                <div className="w-full flex items-end justify-between">
                    <div className="w-full">
                        ...
                        <h6 className="text-sm font-semibold text-gray-800 dark:text-gray-100">
                            {moment(reservation.hotel.first_check_in_at).format('DD MMMM YYYY')} - {moment(reservation.hotel.last_check_out_at).format('DD MMMM YYYY')}
                        </h6>
                        <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">({reservation.amount} {__('packages')}) &middot; {reservation.hotel.program_type} days Program</p>
                        ...
                        <div className="mb-2">
                            <p className="text-sm text-gray-700 dark:text-gray-200 font-medium">
                                Room Detail:<br/>
                                {reservation.hotel.room_detail.quad} Quad Rooms<br/>
                                {reservation.hotel.room_detail.triple} Triple Rooms<br/>
                                {reservation.hotel.room_detail.double} Double Rooms<br/>
                            </p>

...
const Pool = () => {
    ...
    const getFlights = async () => {
        setLoading(true)
        await axios.get("/hajj-umrah/flights/pool").then(res => {
            setLoading(false)
                ...
                loading
                    ...
                    : (
                        <div className="w-full">
                            <div className="flex flex-col md:flex-row md:justify-between mb-2 w-full">
                                <h5 className="text-gray-900 dark:text-white text-lg font-semibold mb-2 md:mb-0">{__('title.hajj_umrah.flights')}</h5>
                                <Input type="text" id="search" name="search" label="" containerClassName="w-full md:max-w-xl" className="!rounded-full placeholder:opacity-100" placeholder={__('search')} value={query} onChange={(e) => setQuery(e.target.value)} />
                            </div>
                            ...
                            {
                                selectedFlight && (
                                    <BookFlight user={user} pool flight={selectedFlight} setSelectedFlight={setSelectedFlight} />
                                )
                            }
                        </div>
                    )

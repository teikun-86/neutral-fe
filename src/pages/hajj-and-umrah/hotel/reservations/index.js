...
import { HotelReservation } from '@/components/hotel/hajj-umrah/reservation';
...
const Reservations = () => {
    ...
    const queriedReservations = query.trim().length === 0 ? reservations : reservations.filter(reservation => {
        return searchString(query, reservation.hotel.company ? reservation.hotel.company.name : '')
            || searchString(query, reservation.hotel.location_1)
            || searchString(query, reservation.hotel.location_2)
            || searchString(query, reservation.hotel.price_per_package)
            || searchString(query, reservation.hotel.room_detail.quad)
            || searchString(query, reservation.hotel.room_detail.triple)
            || searchString(query, reservation.hotel.room_detail.double)
            || searchString(query, moment(reservation.hotel.first_check_in_at).format('DD MMMM YYYY'))
            || searchString(query, moment(reservation.hotel.first_check_out_at).format('DD MMMM YYYY'))
            || searchString(query, moment(reservation.hotel.last_check_in_at).format('DD MMMM YYYY'))
            || searchString(query, moment(reservation.hotel.last_check_out_at).format('DD MMMM YYYY'))
    })

    const getReservations = async () => {
        setLoading(true)
        await axios.get("/hajj-umrah/hotels/reservations").then(res => {
            setLoading(false)
...

                            {
                                queriedReservations.length === 0
                                    ...
                                    : (
                                        <div className="w-full">
                                            {
                                                queriedReservations.map(reservation => <HotelReservation res={reservation} key={reservation.id} setParentReservation={setReservation} reload={getReservations} />)
                                            }
                                        </div>
                                    )
                            }
